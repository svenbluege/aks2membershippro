...
class os_paypal extends MPFPayment
{
	...
	public function verifyRecurringPayment()
	{
		...
		if ($ret)
		{
			...
			// Hack to support routing from Akeeba Subscription to Membership Pro
			$akeeba = JRequest::getInt('akeebasubs', 0);
			if ($akeeba == 1)
			{
				$db = JFactory::getDbo();
				$query = $db->getQuery(true);
				$query->select('id')
						...
						->order('id');
				$db->setQuery($query);
				$akeebaSubscriptionId = (int) $db->loadResult();
				if ($akeebaSubscriptionId)
				{
					$id = $akeebaSubscriptionId;
				}
			}
